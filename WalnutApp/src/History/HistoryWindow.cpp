...
#include "../Utils/DateFormatting.h"
...
void HistoryWindow::Render(TabManager& tabManager, HistoryManager& history, ImGuiID dockspaceId)
{
	...
	{
		std::string dayLabel = m_entries[i].lastVisit > 0 ? DateFormatting::GetDayLabel(m_entries[i].lastVisit) : "Unknown date";
