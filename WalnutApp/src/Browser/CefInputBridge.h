...
/// CefInputBridge — stateless helpers that translate ImGui input events
/// into CEF mouse / keyboard events.
///
/// Lives in WalnutApp (not in core Walnut) because it couples ImGui
/// types with CEF types — a dependency the core library should not have.
...
namespace CefInputBridge {
	...
	inline uint32_t GetCefModifiers()
	{
		...
		if (io.MouseDown[0]) modifiers |= EVENTFLAG_LEFT_MOUSE_BUTTON;
		if (io.MouseDown[1]) modifiers |= EVENTFLAG_RIGHT_MOUSE_BUTTON;
		if (io.MouseDown[2]) modifiers |= EVENTFLAG_MIDDLE_MOUSE_BUTTON;
		return modifiers;
	}
	...
	/// Forward all pending mouse events (move, click, wheel) to the browser.
	/// `mouseX` / `mouseY` are relative to the browser image origin.
	...
	/// Forward all pending keyboard events (char input + key down/up) to the
	/// browser.
